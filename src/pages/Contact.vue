...
export default {
  ...
  computed: {
    phone() {
      return this.info.contact.phones[0].number;
    },
    phoneLink() {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      this.submitMessage = "";
      try {
        const [firstName, ...remainingName] = this.form.fullName
          .trim()
          .split(/\s+/);
        const result = await emailService.sendContactEmail({
          ...this.form,
          firstName,
          lastName: remainingName.join(" ") || "Patient",
...
<template>
  <main class="contact-page">
    <PageHero
      ...
      description="64 Oriola Street, Alapere, Lagos 100001, Nigeria — with free on-site parking and a 24-hour emergency department."
    />

    <section class="contact-intro section section--mist">
      <div class="site-container">
        <a class="contact-callout" :href="phoneLink"
          ><i class="bi bi-telephone-fill"></i><span>{{ phone }}</span
          ><small>Book an appointment</small></a
        >
        <div class="contact-options">
          <a :href="info.address.gmap" target="_blank" rel="noopener"
            ><i class="bi bi-geo-alt"></i
            ><span
              ><strong>Visit us</strong
              ><small>64 Oriola Street<br />Alapere, Ketu, Lagos</small></span
            ><b>Get directions <i class="bi bi-arrow-up-right"></i></b
          ></a>
          <a :href="phoneLink"
            ><i class="bi bi-telephone"></i
            ><span
              ><strong>Call us</strong
              ><small>{{ phone }}<br />Front desk & emergencies</small></span
            ><b>Call now <i class="bi bi-arrow-up-right"></i></b
          ></a>
          <a :href="`mailto:${info.contact.emails[0].address}`"
            ><i class="bi bi-envelope"></i
            ><span
              ><strong>Email us</strong
              ><small
                >{{ info.contact.emails[0].address }}<br />{{
                  info.contact.emails[1].address
                }}</small
              ></span
            ><b>Send email <i class="bi bi-arrow-up-right"></i></b
          ></a>
          <a :href="whatsappLink" target="_blank" rel="noopener"
            ...
            ><span
              ...
              ><small
                >Quick questions & bookings<br />Replies during clinic
                hours</small
              ></span
            ><b>Open chat <i class="bi bi-arrow-up-right"></i></b
          ></a>
        </div>
      </div>
    </section>
      ...
      <div class="site-container contact-grid">
        <div class="contact-form-wrap">
          <div class="section-heading">
            ...
            <p>
              For non-urgent enquiries. In an emergency, please call
              <a :href="phoneLink">{{ phone }}</a> or come straight to the
              hospital.
            </p>
          </div>
          <p
            v-if="submitMessage"
            class="form-message"
            :class="{ 'form-message--error': !submitSuccess }"
            role="status"
          >
            {{ submitMessage }}
          </p>
          <form @submit.prevent="submitForm">
            ...
            <label
              >Subject *<input v-model.trim="form.subject" required
            /></label>
            <label
              >Message *<textarea
...
.contact-options > a:hover {
  ...
  box-shadow: 0 12px 26px rgba(23, 72, 103, 0.1);
  border-color: #9bd2e7;
}
...
.contact-form-wrap textarea:focus {
  border-color: #58afcf;
  box-shadow: 0 0 0 3px rgba(45, 145, 201, 0.13);
}
