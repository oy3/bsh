      ...
      <div class="site-container detail-layout">
        <article>
          ...
          <div class="feature-grid">
            <article v-for="feature in service.features" :key="feature.title" class="feature-card"><i
          ...
          <div v-if="relatedDoctors.length" class="specialists">
            ...
            <div class="specialist-grid"><router-link v-for="doctor in relatedDoctors" :key="doctor.id"
                :to="{ name: 'DoctorDetail', params: { id: doctor.id } }" class="specialist-card"><img
                  v-if="doctor.image" :src="doctor.image" :alt="doctor.name">
                <div v-else class="specialist-placeholder"><i class="bi bi-person"></i></div>
                <div><span>{{ doctor.division }}</span>
                  <h3>{{ doctor.name }}</h3>
                  <p>{{ doctor.role }}</p>
                </div>
              </router-link></div>
        ...
        <aside class="detail-aside">
          <div class="appointment-card"><i class="bi bi-calendar2-check"></i>
            <h2>Speak to our team</h2>
            <p>Request an appointment and we will help you find the right next step.</p><router-link
              to="/book-appointment" class="button-primary">Request appointment <i
                class="bi bi-arrow-right"></i></router-link>
          </div>
          <div class="emergency-card"><i class="bi bi-heart-pulse"></i>
            <h2>Need urgent care?</h2>
            <p>Our emergency department is open 24 hours, every day.</p><a
              :href="`tel:${info.contact.phones[0].number.replace(/\s/g, '')}`">{{ info.contact.phones[0].number }}</a>
          </div>
          <nav class="other-services" aria-label="Other services">
            <h2>Explore other services</h2><router-link v-for="item in otherServices" :key="item.id"
              :to="{ name: 'ServiceDetail', params: { id: item.id } }"><i :class="['bi', item.icon]"></i>{{ item.title
...
.detail-intro {
  ...
  color: var(--muted);
  margin: 0 0 2.3rem;
  animation: detail-enter .5s both
}
...
.feature-card {
  ...
  border-radius: 17px;
  animation: detail-enter .55s both
}
...
.appointment-card .button-primary {
  width: 100%;
  font-size: .78rem;
  padding: .75rem
}
...
.other-services a>i:first-child {
  color: var(--aqua)
}

.other-services a>i:last-child {
  font-size: .7rem;
  color: var(--muted);
  transition: transform .2s
}

.other-services a:hover>i:last-child {
  transform: translateX(3px)
}
...
.specialist-card:hover {
  ...
  box-shadow: 0 12px 22px rgba(10, 54, 84, .1)
}
